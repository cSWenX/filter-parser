...
const ImageEnhancement: React.FC<ImageEnhancementProps> = ({ className = '' }) => {
  ...
  const handleApplyFilter = async (filter: ParameterHistory) => {
    ...
    try {
      ...
      // 设置处理后的图片预览URL
      const previewUrl = `http://localhost:8080${result.preview_url}?t=${Date.now()}`;
  ...
  const handleDownloadImage = () => {
    if (!enhancedImageUrl || !selectedFilter) {
    ...
    try {
      // 从预览URL中提取output_id
      const previewUrl = new URL(enhancedImageUrl);
      const pathParts = previewUrl.pathname.split('/');
      const outputId = pathParts[pathParts.length - 1];

      // 构建下载URL
      const downloadUrl = `http://localhost:8080/api/download/${outputId}`;
